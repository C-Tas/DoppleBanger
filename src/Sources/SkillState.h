...
protected:
	...
	///<summary>Actualiza los iconos de las skills (los que están equipados)</summary>
	...
	///<summary>Método auxiliar para el callback de equipar la skill a las teclas
	///0->q, 1->w, 2->e </summary>
	void equipSelectedSkill(SkillEquiped key);

	///<summary>Métodos auxiliares para el feedback visual</summary>
	void meleePointsActualized();
	void distancePointsActualized();
	void ghostPointsActualized();

	///<summary>Método auxiliar que cambia de texturas cuando las habilidades se desbloquean
	...
	///Método que, dado el nombre de la skill, devuelve su icono correspondiente
	///Se usa al pintar las skills que están equipadas
	...
	//<summary>Método que dado el nombre de una skill crea un objeto Skill* correspondiente
	///FALTA IMPLEMENTAR LOS OBJETOS QUE HEREDEN DE SKILL, PERO LA ESTRUCTURA ESTÁ</summary>
	Skill* createSkill(SkillNames name);
public:
	///<summary>Constructora de CreditsState</summary>
	SkillState(Application* app = nullptr, Player* player = nullptr) :GameState(app), player_(player) { initSkillState(); };
	...
	///<summary>Método para modificar la descripcion que aparece en pantalla</summary>
