...
struct Stats
{
	...
	double Mana = 0;		//Maná de la entidad
	...
	Stats(double health, double mana, double manaReg, double armor, double meleeDmg, double distDmg,
		...
		health_(health), Mana(mana), manaReg_(manaReg), armor_(armor), meleeDmg_(meleeDmg), distDmg_(distDmg), 
		crit_(crit), meleeRange_(meleeRange), distRange_(distRange), moveSpeed_(moveSpeed), meleeRate_(meleeRate), distRate_(distRate) {};

	Stats() { health_, Mana, manaReg_, armor_, meleeDmg_, distDmg_, crit_, meleeRange_, moveSpeed_, meleeRate_, distRate_ = 0; }
};