...
void callSelectObjectS(GameState* state, Equipment* ob) {
	dynamic_cast<ShopState*>(state)->selectObject(ob->getButton());
}
...
ShopState::ShopState(Application* app, Player* player) :GameState(app), player_(player)
{
	...
	advanceButton_ = new Button(app, this, app_->getTextureManager()->getTexture(Resources::TextureId::ForwardButton), Vector2D{ 700,100 }, Vector2D{ 50,50 }, callForwardListS);
	...
	exitButton_ = new Button(app, app_->getTextureManager()->getTexture(Resources::TextureId::Timon), Vector2D{ 1300,100 }, Vector2D{ 50,50 }, callExitS);
	addUpdateList(exitButton_);
	addRenderList(exitButton_);

	buyButton_ = new Button(app, this, app_->getTextureManager()->getTexture(Resources::TextureId::MenuFondo), Vector2D{ 50,700 }, Vector2D{ 50,50 }, callBuyObjectS, 1);
	addUpdateList(buyButton_);
	addRenderList(buyButton_);
	shopAdvanceButton_ = new Button(app, this, app_->getTextureManager()->getTexture(Resources::TextureId::ForwardButton), Vector2D{ 300,100 }, Vector2D{ 50,50 }, callShopForwardListS, 1);
...
void ShopState::addToInventory(Equipment* ob)
{
	//creamos un boton
