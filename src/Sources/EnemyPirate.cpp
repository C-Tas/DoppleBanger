...
bool EnemyPirate::onRange() {
	...
	/*cout << meleePosX << " / " << meleePosY << "-----"<< meleeRangeX << " / " << meleeRangeY << endl;
	cout << "----------------------------------" << endl;
	cout << pos_.getX() << " / " << pos_.getY() << "-----" << getScaleX() << " / " << getScaleY() << endl;*/
...
void EnemyPirate::attack() {
	...
	else if(currStats_.meleeRate_ <= SDL_GetTicks() - lastMeleeHit_)
	{
		...
		/*Bullet* bullet = new Bullet(app_, app_->getTextureManager()->getTexture(Resources::Coco),
			getCenter(pos_), currEnemy_->getPos(), currStats_.distDmg_, true);
		app_->getCurrState()->addRenderUpdateLists(bullet);
		CollisionCtrl::instance()->addEnemyBullet(bullet);*/
